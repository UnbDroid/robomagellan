...
public:
    ...
    int depth() const {
        int n = 0;
        for (Node<T>* ancestor = _parent; ancestor != NULL;
...
public:
     ...
     * @state.
     */
    Node<T>* nearest(const T& state, float* distanceOut = NULL) {
     ...
     * @param dest The node in the tree to get the path for. If NULL, will
    ...
    void getPath(std::function<void(const T& stateI)> callback,
                 const Node<T>* dest = NULL, bool reverse = false) const {
        const Node<T>* node = (dest != NULL) ? dest : lastNode();
        if (reverse) {
            while (node) {
                callback(node->state());
                node = node->parent();
     ...
     */
    void getPath(std::vector<T>* vectorOut, const Node<T>* dest = NULL,
     ...
     */
    std::vector<T> getPath(const Node<T>* dest = NULL,
                           bool reverse = false) const {
        std::vector<T> path;
    ...
    /**
     * @return The root node or NULL if none exists
     */
    ...
    void setStartState(const T& startState) {
        ...
        _nodes.push_back(Node<T>(startState, NULL));
    }
