...
void RastreiaCone::Filtrar(Mat *src )
{
	...
	//Filtragem
	Mat element0 = getStructuringElement( MORPH_ELLIPSE,Size( 3, 3 ),Point( 1, 1 ) );
	erode(*src,*src,element0);
	imshow("erode",*src);
	medianBlur(*src,*src,3);
	imshow("median",*src);
	dilate( *src, *src, element0 );
	imshow("emd",*src);
...
double RastreiaCone::rastreia(Mat source, bool pred, float  X, float Y, float T, float varX, float varY){
	...
	*/
	...
	if(false)
	{
		...
		//predxd = aux*(1+a1/30);
		//predxe = aux*(1+a2/30);
		predxd = aux*(1+a1/35.29);
		predxe = aux*(1+a2/35.29);
		...
		else
		{
			...
			*/
			...
			else
			{
				...
				yi = linha_centro - aux-5;
				yf = linha_centro + aux+5;
		...
		if(good_pred)
		{
			...
			*/
			...
			Identifica(source.cols,70.58,source.rows,55.92,razao);
	...
	if(true)
	{
		...
		//source.copyTo(temp2);
		//imshow("area",temp2);
		resize(source, temp2, Size((int)(((float)480)*ratio_original), 480 ));
		objetos.clear();
		cones.clear();		
		Mat temp(temp2.size(),CV_8UC1);
		//blur(temp2(roi),temp2(roi),Size(3,3));
	...
	if(cones.size()>0)
	{
		...
		variancia = 2*razao/(float)(cones[best_cone].fim.y-cones[best_cone].inicio.y);
			...
			*/
			...
			putText(mostra, "X: "+num, Point(20,40), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255,255,0), 1, 8, false );
			num.clear();
			//float_to_string(cones[best_cone].Y,&num,5);
			
			float_to_string(Y,&num,5);
			//putText(mostra, "Y: "+num, Point(10,30), FONT_HERSHEY_SIMPLEX, 0.25, Scalar(255,255,0), 1, 8, false );
			putText(mostra, "Y: "+num, Point(20,60), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255,255,0), 1, 8, false );
		...
		else
		{
			...
			putText(mostra, "X: -", Point(20,40), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255,255,0), 1, 8, false );
			putText(mostra, "Y: -", Point(20,60), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255,255,0), 1, 8, false );
		}
		
		num.clear();
		float_to_string(T,&num,5);
		//putText(mostra, "T: "+num, Point(10,10), FONT_HERSHEY_SIMPLEX, 0.25, Scalar(255,255,0), 1, 8, false );
		putText(mostra, "T: "+num, Point(20,20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255,255,0), 1, 8, false );
		if(pred){
			//cout<<varX<<"    "<<varY<<endl;
			rectangle(mostra,Point(predxe,predyt),Point(predxd,predyb),Scalar(0,0,255));
			//circle(mostra,Point(predxe,predyt),7,Scalar(0,255,0));
			//circle(mostra,Point(predxd,predyb),7,Scalar(255,0,0));
			rectangle(mostra,Point((int)xi,(int)yi),Point((int)xf,(int)yf),Scalar(0,255,0));
			//circle(mostra,Point(xi,yi),14,Scalar(0,255,0));
			//circle(mostra,Point(xf,yf),14,Scalar(255,0,0));
