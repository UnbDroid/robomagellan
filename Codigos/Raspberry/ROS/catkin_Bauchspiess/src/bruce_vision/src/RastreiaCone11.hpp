...
int RastreiaCone::varredura0(Mat* source){
	...
	inRange( aux2 , Scalar(20), Scalar(257), aux2);
	#ifdef usuario
		#ifdef mostra_range
			imshow("Regiao de Interesse", aux2);
		#endif
	#endif
	blur((*source), aux3, Size(Peso_original,Peso_original));
	#ifdef usuario
		#ifdef mostra_range
			imshow("BLur original", aux3);
		#endif
	#endif
	inRange( aux3 , Scalar(Hist_final), Scalar(257), aux3);
	#ifdef usuario
		#ifdef mostra_range
			imshow("Pontos novos", aux3);
		#endif
	#endif
	bitwise_and(aux2,aux3, aux3);
	#ifdef usuario
		#ifdef mostra_range
			imshow("Aceitos 1", aux3);
		#endif
	#endif
