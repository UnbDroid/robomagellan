	...
	for(i = 0; i<256; i++){
		for(j = 0; j<256; j++){
			for(k = 0; k<256; k++){
				...
				#endif
				#ifndef soma
					Tabela[i][j][k] = (uchar) 255*H[i]*H[i]*S[j]*S[j]*S[j]*S[j]*V[k];
				#endif
			}
		}
	}
...
void RastreiaCone::Laranja(Mat source, Mat* dest, int vez){
	...
	split(hsv, channels);
		...
		float H_hist = (float)H_pondera/100;
		float H_regular = 1-H_hist;
		float S_hist = (float)S_pondera/100;
		float S_regular = 1-S_hist;
		float V_hist = (float)V_pondera/100;
		float V_regular = 1-V_hist;

		add(H_regular*channels[0], H_hist*canais[0], fusao[0]);
		add(S_regular*channels[1], S_hist*canais[1], fusao[1]);
		add(V_regular*channels[2], V_hist*canais[2], fusao[2]);
	#endif	
		...
		Mat mostra_imagem;
		#ifdef regular
			Mat partes_mostra0(source.size(), CV_8UC1);
			Mat partes_mostra1(source.size(), CV_8UC1);
			Mat partes_mostra2(source.size(), CV_8UC1);
		#endif
		#ifdef histograma	
			Mat partes_mostra3(source.size(), CV_8UC1);
			Mat partes_mostra4(source.size(), CV_8UC1);
			Mat partes_mostra5(source.size(), CV_8UC1);
		#endif
		#ifdef ponderado	
		    ...
		    for( i = 0; i < nRows; ++i)
		    {
		    	p2 = partes_mostra3.ptr<uchar>(i);
		        p = canais[0].ptr<uchar>(i);
		        for ( j = 0; j < nCols; ++j)
		        {
		            p2[j] = 255*H[p[j]];
		        }
		    ...
		    for( i = 0; i < nRows; ++i)
		    {
		    	p2 = partes_mostra4.ptr<uchar>(i);	
		        p = canais[1].ptr<uchar>(i);
		        for ( j = 0; j < nCols; ++j)
		        {
		            p2[j] = 255*S[p[j]];
		        }
		    ...
		    for( i = 0; i < nRows; ++i)
		    {
		    	p2 = partes_mostra5.ptr<uchar>(i);
		        p = canais[2].ptr<uchar>(i);
		        for ( j = 0; j < nCols; ++j)
		        {
		            p2[j] = 255*V[p[j]];
		        }
	    ...
	    if(vez == VEZ){
	    	#ifdef regular
		    	cvtColor(partes_mostra0, mostra_imagem, CV_GRAY2BGR);
		    	putText(mostra_imagem, ("H"), Point(0,20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255,0,0), 1, 8, false );
		    	imshow("Hdepois1", mostra_imagem);
		    	cvtColor(partes_mostra1, mostra_imagem, CV_GRAY2BGR);
		    	putText(mostra_imagem, ("S"), Point(0,20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255,0,0), 1, 8, false );
		    	imshow("Sdepois1", mostra_imagem);
		    	cvtColor(partes_mostra2, mostra_imagem, CV_GRAY2BGR);
		    	putText(mostra_imagem, ("V"), Point(0,20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255,0,0), 1, 8, false );
		    	imshow("Vdepois1", mostra_imagem);
	    	#endif

		    #ifdef histograma	
		    	cvtColor(partes_mostra3, mostra_imagem, CV_GRAY2BGR);
		    	putText(mostra_imagem, ("H"), Point(0,20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255,0,0), 1, 8, false );
		    	imshow("Hdepois2", mostra_imagem);
		    	cvtColor(partes_mostra4, mostra_imagem, CV_GRAY2BGR);
		    	putText(mostra_imagem, ("S"), Point(0,20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255,0,0), 1, 8, false );
		    	imshow("Sdepois2", mostra_imagem);
		    	cvtColor(partes_mostra5, mostra_imagem, CV_GRAY2BGR);
		    	putText(mostra_imagem, ("V"), Point(0,20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255,0,0), 1, 8, false );
		    	imshow("Vdepois2", mostra_imagem);
		    #endif	
...
int RastreiaCone::varredura0(Mat* source){
	...
	for(int i = 0; i<Hist_final+1; i++){			
		...
		#ifdef usuario
			#ifdef mostra_range
				//imshow("Range"+num, aux);
			#endif
		#endif
		blur(aux, aux2, Size(20,20));
		#ifdef usuario
			#ifdef mostra_range
				//imshow("Blur"+num, aux2);
			#endif
		#endif
		float peso_blur = (float)Peso_blur/100.0;
		float peso_original = (float)Peso_original/100.0; 
		add(peso_blur*aux2, peso_original*(*source), aux2);
		#ifdef usuario
			#ifdef mostra_range
				//imshow("ADD"+num, aux2);
			#endif
	...
	inRange( aux2 , Scalar(min_aceita), Scalar(257), aux2);
	#ifdef usuario
		#ifdef mostra_range
			imshow("RangeFinal", aux2);
		#endif
	#endif

	/*		
	blur(aux2, aux, Size(1,4));
	#ifdef usuario
		#ifdef mostra_range
			imshow("BlurElimina", aux);
		#endif
	#endif
	inRange( aux , Scalar(240), Scalar(257), (*source));
	#ifdef usuario
		#ifdef mostra_range
			imshow("RangeElimina", (*source) );
		#endif
	#endif
	*/

	/*
	blur(hist2, hist3, Size(10,10));
	imshow("BLURhist", hist3);
	add(hist3, hist1, hist3);
	imshow("Hist2", hist3);
	//blur(hist1, hist2, Size(1,4));
	inRange( hist2 , Scalar(253), Scalar(257), hist1);
	imshow("Bom2", hist1);
	*/
...
bool RastreiaCone::varredura1(Mat* source, int num_area){
	...
	for(int i = 0; i<Hist_final+1; i++){			
		...
		#ifdef usuario
			#ifdef mostra_range
				//imshow("Range"+num, aux2);
			#endif
		#endif
		bitwise_or(aux2, (*source), (*source));
		#ifdef usuario
			#ifdef mostra_range
				//imshow("Or"+num, (*source));
			#endif
		#endif
	}
	inRange( aux2 , Scalar(min_aceita), Scalar(257), aux2);
	...
	*/
	...
	Mat aux2;
	inRange( (*source) , Scalar(min_aceita), Scalar(257), aux);
	blur(aux, aux2, Size(3,8));
	add(aux2, (*source), aux2);
	inRange( aux2 , Scalar(min_aceita), Scalar(257), (*source));
	//imshow("outro", (*source));
	blur((*source), aux, Size(1,4));
	//imshow("Blur", aux);
	inRange( aux , Scalar(180), Scalar(257), (*source));
