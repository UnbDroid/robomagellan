...
void RastreiaCone5::Laranja(Mat source, Mat* dest, int vez){
	...
	*/
	...
	vector<Mat> channels;
	vector<Mat> canais(3);
	vector<Mat> canais_final(3); 
	Mat aux_mat(hsv.size(), CV_8UC3);
	split(hsv,channels);
	inRange(channels[0], Scalar(min_Hb), Scalar(max_Hb), canais[0]);
	inRange(channels[0], Scalar(min_Hm), Scalar(max_Hm), canais[1]);
	inRange(channels[0], Scalar(min_Hr), Scalar(max_Hr), canais[2]);
	//merge(canais,aux_mat);
	bitwise_or(canais[0], 0.5*canais[1], canais_final[0]);
	bitwise_or(canais_final[0], 0.25*canais[2], canais_final[0]);;
	#ifdef usuario 
	//imshow("Hue", canais_final[0]);
	#endif
	inRange(channels[1], Scalar(min_Sb), Scalar(max_Sb), canais[0]);
	inRange(channels[1], Scalar(min_Sm), Scalar(max_Sm), canais[1]);
	inRange(channels[1], Scalar(min_Sr), Scalar(max_Sr), canais[2]);
	//merge(canais,aux_mat);
	bitwise_or(canais[0], 0.5*canais[1], canais_final[1]);
	bitwise_or(canais_final[1], 0.25*canais[2], canais_final[1]);
	

	//imshow("Saturation", canais_final[1]);
	

	inRange(channels[2], Scalar(min_Vb), Scalar(max_Vb), canais[0]);
	inRange(channels[2], Scalar(min_Vm), Scalar(max_Vm), canais[1]);
	inRange(channels[2], Scalar(min_Vr), Scalar(max_Vr), canais[2]);
	//merge(canais,aux_mat);
	bitwise_or(canais[0], 0.5*canais[1], canais_final[2]);
	bitwise_or(canais_final[2], 0.25*canais[2], canais_final[2]);
	

	//imshow("Value", canais_final[2]);

	//canais_final[0].convertTo(canais_final[0],CV_32FC1, 1/255.0);
	//canais_final[1].convertTo(canais_final[1],CV_32FC1, 1/255.0);
	//canais_final[2].convertTo(canais_final[2],CV_32FC1, 1/255.0);
	//imshow("Hue", canais_final[0]);
	//imshow("Saturation", canais_final[1]);
	//imshow("Value", canais_final[2]);

	//multiply(canais_final[1], canais_final[2], aux_mat);
	bitwise_and(canais_final[1], canais_final[2], aux_mat);
	

	//imshow("Mul2", aux_mat);
	

	//multiply(canais_final[0], aux_mat, (*dest));
	bitwise_and(canais_final[0], aux_mat, (*dest));
...
int RastreiaCone5::varredura0(Mat* source){
	...
	#endif
	blur( (*source), aux, Size(6,2 ));
	#ifdef usuario
	imshow("Blur", aux);
	#endif
	inRange(aux, Scalar(130), Scalar(255), aux);
	#ifdef usuario
	imshow("Range", aux);
	#endif
	/*
...
bool RastreiaCone5::varredura1(Mat* source, int num_area){
	...
	inRange(aux, Scalar(130), Scalar(255), (*source));
	/*
	canais.clear();
	canais.push_back(aux);
	canais.push_back( (*source) );
