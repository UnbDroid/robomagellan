...
void RastreiaCone::PreencheTabela(){
	...
	float tabelaHV[256][256];
	float tabelaSV[256][256];
	float tabelaHS[256][256];
...
void RastreiaCone::Laranja(Mat source, Mat* dest, int vez){
	...
	Mat mostra;
	cvtColor(hsv, mostra, CV_HSV2BGR);
	string num;
	int_to_string(vez, &num);
	imshow("cor nova"+num, mostra);
...
bool RastreiaCone::dados_mancha(Mat mancha, int partes){
	...
	int tamanho = mancha.rows/(partes+2);
