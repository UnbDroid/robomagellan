...
*/
    ...
    cvtColor(source, img_hist_equalized, CV_BGR2YCrCb); //change the color image from BGR to YCrCb format
    split(img_hist_equalized,channels); //split the image into channels
    equalizeHist(channels[0], channels[0]); //equalize histogram on the 1st channel (Y)
  	merge(channels,img_hist_equalized); //merge 3 channels including the modified 1st channel into one image
   	cvtColor(img_hist_equalized, img_hist_equalized, CV_YCrCb2BGR);

   	cvtColor(img_hist_equalized, hsv, CV_BGR2HSV);
...
bool RastreiaCone5::varredura1(Mat* source, int num_area){
	...
	blur( (*source), aux, Size(2,4 ));
	inRange(aux, Scalar(150), Scalar(255), (*source));
	/*
	canais.clear();
	canais.push_back(aux);
	canais.push_back( (*source) );
...
bool RastreiaCone5::dados_mancha(Mat mancha, int partes){
	...
	cont_verdadeiros = cont_verdadeiros/comparacoes;
	cont_quase = cont_quase/comparacoes;
	...
	*/
	...
	if( (cont_quase > 0.6)&&(cont_verdadeiros> 0.8)&&( (float)mancha.rows > (float)mancha.cols) ){
		continuidade = true;
	}
