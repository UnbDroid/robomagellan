...
void RastreiaCone::Laranja(Mat source, Mat* dest, int vez){
    ...
    //if(vez == 2)
    imshow("Vdepois", channels[2]);

    #endif

    int num_channels = hsv.channels();

    nRows = hsv.rows;
    nCols = hsv.cols * num_channels;

    Mat final(hsv.size(), CV_8UC1);
