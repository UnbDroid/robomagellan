...
int RastreiaCone::varredura0(Mat* source){
	...
	//equalizeHist((*source), hist1);
	//imshow("histograma", hist1);
	#endif
	#endif
	inRange( (*source) , Scalar(min_aceita), Scalar(257), aux);
	//inRange( hist1 , Scalar(255), Scalar(257), hist2);
	//imshow("Bom", hist2);
	#ifdef usuario
	#endif
	blur(aux, aux2, Size(8,2));
