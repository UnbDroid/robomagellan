...
void RastreiaCone::rastreia(Mat source, bool pred, float  X, float Y, float T, float varX, float varY){
	...
	*/
	...
	//if(false)
	{
		...
		beta = (180.0/CV_PI)*atan(0.20/dist);
		a1 = alpha+beta/2;
		a2 = alpha-beta/2;
		predxd = aux*(1+a1/30);
		predxe = aux*(1+a2/30);
		...
		if(good_pred){
			...
			for(i = 0; i<objetos.size();i++)
			{
				for(j=0;j<objetos[i].num_pontos;j++)
				{
					objetos[i].pontos[j].x = (int)((float)objetos[i].pontos[j].x*razao)-x;
					objetos[i].pontos[j].y = (int)((float)objetos[i].pontos[j].y*razao)-y;
				}			
			}
	...
	if(cones.size()>0)
	{
		...
		#if defined(filmar)||defined(usuario)
			rectangle(mostra,cones[best_cone].inicio,  cones[best_cone].fim,Scalar(255,0,0 ) );
	...
	#ifdef filmar
	if(!video_init){
		//video = new VideoWriter("/home/pi/Desktop/Receiver/Camera.avi",CV_FOURCC('M','J','P','G'),10, Size(temp2.cols,temp2.rows),true);
